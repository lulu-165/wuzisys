...
class Core
{
    ...
    function Route()
    {
        ...
        if (!empty($_GET['url'])) {
            ...
            // 获取控制器名
            $controllerName = $urlArray[0];//首字母大写函数ucfirst
            // 获取动作名
		...
		//echo '  controllerName='. $controllerName.'<br>'.'  action='. $action.'<br><br>';

        // 如果控制器存和动作存在，这调用并传入URL参数
        if ((int)method_exists($controller, $action)) {//判断某个类中是否包含某个方法
			$dispatch = new $controller($controllerName, $action);
			//print_r(array($dispatch, $action)) ;exit;
            call_user_func_array(array($dispatch, $action), $queryString);//动态调用普通函数，比如参数和调用方法名称不确定的时
        } else {
			...
			$viewObj = new View($controllerName, $action);
			$viewObj->render();
        }
    ...
    static function loadClass($class)
    {
        ...
        $controllers = APP_PATH . 'mvc/controllers/' . $class . '.class.php';
        $models = APP_PATH . 'mvc/models/' . $class . '.class.php';
