...
<script>
    ...
    $(".addUnit").live("click", function(e) {
        ...
        $tr = '<tr id="addTR">';
        $tr += '<td style="width:229px;"><select id="addCangku" style="display: block;"  name="selectStore" lay-filter="selectStore"> <option  value="all">选择仓库</option></select></td>>';
        $tr += '<td style="width:229px;"><select id="class1" style="display: block;"></select></td>';
        ...
        $tr += '<td style="width:154px;"><input id="addWuziName" value="" placeholder="输入物资名称" /></td>';
        $tr += '<td style="width:113px;"><input id="addWuziCode" type="number" value="" placeholder="输入物资编码" /></td>';
        $tr += '<td style="width:124px;"><input id="addAlertVal_min" type="number" value="" placeholder="预警下限值" /></td>';
        $tr += '<td style="width:124px;"><input id="addAlertVal_max" type="number" value="" placeholder="预警上限值" /></td>';
        $tr += '<td style="padding-left:9px !important;"><button style="margin:2px 0 2px;" type="button" id="saveUnit" class="layui-btn layui-btn-sm">保存</button><br/><button type="button" id="cancelAdd" class="layui-btn layui-btn-danger layui-btn-sm">取消</button></td></tr>';
    ...
    $(".del").live("click", function(e) {
        var wId = $(this).parents("tr").find(".cangku").attr("wId");

        layer.confirm('真的删除该行么', function(index) {
            $(this).parents("tr").remove();
            //var obj.parents("tr").find(".cangku").attr("ckId");
            // console.log(obj);
            // return;
            ...
            $.ajax({
                ...
                dataType: 'json',
                url: "<?= CURRENT_DIR ?>/setWZWarning_del.php",
                success: function(msg) {
                    console.log(msg);
                    if (msg != "error") {
    ...
    $(".update").live("click", function(e) {
        var wId = $(this).parents("tr").find(".cangku").attr("wId");
        var warningVal_min = $(this).parents("tr").find(".min").val();
        var warningVal_max = $(this).parents("tr").find(".max").val();
        ...
        $.ajax({
            ...
            dataType: 'json',
            url: "<?= CURRENT_DIR ?>/setWZWarning_update.php",
            success: function(msg) {
                console.log(msg);
                if (msg != "error") {
                    layer.msg('更新成功', {
